...
#define p t->rPtr
  ...
  if ((BYTE_BIT - 1) * o + bits((int)(sgn == 0 ? h : ~h & (BYTE_SIGN_BIT - 1)))
      > WORD_BIT + sgn)
    throwPos(t, "number too large %d %d %d", (BYTE_BIT - 1) * o, bits((int)(sgn == 0 ? h : ~h & (BYTE_SIGN_BIT - 1))), WORD_BIT + sgn);
...
#define CHECK_s(op)
#define CHECK_l(op)
#define CHECK_f(op) if (op & 0xf0) throwPos(t, badFlags)
...
#define OPS(a, b, c) CHECK_ ## a(op1); CHECK_ ## b(op2); CHECK_ ## c(op3)
  ...
  #define GET_IMM1 r = (op1 & 1) ? (t->rPtr--, op2) : ((t->rPtr -= 2), 0); \
    n = getNum(t, &sgn)
  #define GET_IMM2 r = (op2 & 1) ? op3 : (t->rPtr--, 0); n = getNum(t, &sgn)
#else /* big endian */
