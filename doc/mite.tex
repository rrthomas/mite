...
Names must be unique within a translation unit. A label is written as
a label type followed by a name, or if the type is given by the
operand type, just a name. A label definition is the same; the
distinction is purely semantic. A name is a string of letters, digits
