...
#define p t->rPtr
  ...
  while (*p && (isspace((char)*p) || (char)*p == '#')) {
    if ((char)*p == '#') do { p++; } while (*p && (char)*p != '\n');
    if ((char)*p == '\n') excLine++;
    p++;
  }
  *tok = (char *)p;
  while (f((char)*p)) p++;
  if (*tok == (char *)p) throw("missing token");
  if (!isspace((char)*p)) throw("bad character");
  if ((char)*p == '\n') t->eol = 1;
  *p = (Byte)'\0';
  return p++ - (Byte *)*tok;
#undef p
...
getOp(TState *t, unsigned int ty)
{
  switch (ty) {
    ...
    case op_b: return getLab(t, LABEL_B);
    case op_s: return getLab(t, LABEL_S);
    case op_d: return getLab(t, LABEL_D);
    case op_i: return getImm(t);
    default: throw("bad operand type");
  }
}
	...
	MiteValue *op1, MiteValue *op2, MiteValue *op3)
{
  ...
  *i = getInstNum(t);
  ...
  if (OP2(ops)) *op2 = getOp(t, OP2(ops));
